...
minimum-maintenance state. The results concern an ideal transfer interface;
they do not by themselves validate a replication protocol or establish
publication priority over all controllable-work scheduling results.
\end{abstract}
...
submission or formal release is represented by this draft. The execution
environment disconnected after a successful 16-page compilation; the
recovered source checkpoint requires a fresh build and visual inspection.
...
Two boundaries clarify what the theorem says. First, monotone smooth
...
The endpoint distinction also matters mathematically. An initially full
module may transfer at once, whereas any strict deficit can face a finite-time
rate barrier. Consequently small state perturbations need not produce small
changes in the exit time at every boundary. The general cold-entry viability
problem and certified numerical frontier evaluation are not solved here.
