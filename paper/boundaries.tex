...
Strict maintenance slack has a simple finite warmup construction. For a
nonzero ready target $z$ with $K=L(z)<s$ and $Q_z=\sum_i z_i$, put
$p_i(t)=\theta(t)z_i$ on maximum loss and choose
\[
 \theta'=\frac{s-K\theta}{Q_z},\quad \theta(0)=0,
 \qquad v_i=z_i(\theta'+\gamma_i\theta).
\]
Until $\theta$ reaches one, these nonnegative allocations sum to $s$.
The hitting time is $(Q_z/K)\log(s/(s-K))<\infty$; positive coefficients
give $K>0$ for a nonzero target. Maintain $z$ thereafter. Scalar comparison
preserves domination under smaller losses. The zero target requires no
warmup. This construction provides no readiness promise before the target
is reached, and it does not cover the critical equality examined next.
